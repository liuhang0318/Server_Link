...
const { sshProgress } = require('./ssh-progress.cjs')
...
class SessionManager {
  ...
  /** 重试沿用同一标签和尺寸，不保留或重放用户输入，也不重连已经认证成功的会话。 */
  startAttempt (record) {
    const { command, environment, id: sessionId } = record
    ...
    // 诊断走独立的 0600 临时日志，不混入终端输出，也不记录用户输入的密码。
    const logDirectory = syncFs.mkdtempSync(path.join(os.tmpdir(), 'serverlink-ssh-progress-'))
    const logPath = path.join(logDirectory, 'connection.log')
    let processHandle
    try {
      syncFs.writeFileSync(logPath, '', { mode: 0o600 })
      processHandle = this.spawn(command.file, ['-v', '-E', logPath, ...command.args], {
        name: 'xterm-256color',
        cols: record.cols,
        rows: record.rows,
        cwd: '/',
      ...
      syncFs.rmSync(logDirectory, { recursive: true, force: true })
      throw error
    }
    ...
    record.exitDisposable = processHandle.onExit(event => {
      this.readProgress(record)
      record.processHandle = null
      // 真正的握手失败在异步退出时才可判定；不能只重试创建 PTY 的 IPC。
      if (!record.closing && !this.shuttingDown && !record.connected && event.exitCode === 255 && record.attempt < 2 && isTransientConnectionError(record.lastLog)) {
        this.clearProgress(record)
        record.dataDisposable?.dispose?.()
        record.exitDisposable?.dispose?.()
        record.attempt++
        record.emit({ type: 'progress', sessionId, phase: 'retrying', attempt: record.attempt, logs: record.lastLog })
        record.retryTimer = setTimeout(() => {
          record.retryTimer = null
          if (record.closing || this.shuttingDown || !this.sessions.has(sessionId)) return
          try {
            this.startAttempt(record)
          } catch (error) {
            record.emit({ type: 'progress', sessionId, phase: 'failed', logs: error.message })
            this.finalize(record)
            record.emit({ type: 'exit', sessionId, exitCode: null, signal: null })
          }
        }, record.attempt * 1000)
    ...
    record.emit({ type: 'status', sessionId, status: 'running' })
    record.emit({ type: 'progress', sessionId, phase: 'connecting', logs: '' })
    record.logTimer = setInterval(() => this.readProgress(record), 200)
    record.logTimer.unref?.()
  }

  /** 限定诊断读取量，认证成功后停止轮询，避免长期会话持续读取文件。 */
  readProgress (record) {
    if (record.closing) return
    try {
      if (syncFs.statSync(record.logPath).size > 256 * 1024) return this.clearProgress(record)
      const log = syncFs.readFileSync(record.logPath, 'utf8')
      if (log === record.lastLog) return
      record.lastLog = log
      const phase = sshProgress(log)
      record.emit({ type: 'progress', sessionId: record.id, phase, logs: log.slice(-16000) })
      if (phase === 'connected') {
        record.connected = true
        this.clearProgress(record)
      }
    } catch {
      // 临时日志不影响 SSH 本身；异常时仍保留终端和关闭操作。
    }
  }

  /** 删除仅由本会话创建的诊断目录，防止断开后留下主机和私钥路径日志。 */
  clearProgress (record) {
    if (record.logTimer) clearInterval(record.logTimer)
    record.logTimer = null
    try { syncFs.rmSync(record.logDirectory, { recursive: true, force: true }) } catch {}
  }
  ...
  write (ownerId, sessionId, data) {
    ...
    // 退避期间没有 PTY，丢弃按键而非排队，避免密码或命令被带入下一次连接。
  ...
  resize (ownerId, sessionId, cols, rows) {
    ...
    // 侧栏动画和重复 fit 可能报告同一字符尺寸，避免多余 SIGWINCH 让远端反复重绘。
  ...
  stop (record) {
    ...
    if (!record.processHandle) {
      this.finalize(record)
      record.emit({ type: 'exit', sessionId: record.id, exitCode: null, signal: null })
      return
    }
    this.clearProgress(record)
    try {
