...
class LocalFiles {
  ...
  async list (ownerId, directoryId = null) {
    ...
    const grant = (filePath, type, uploadable = false) => {
      const id = randomUUID()
      entries.set(id, { path: filePath, type, uploadable })
      return id
    }
    ...
    for (let start = 0; start < dirents.length; start += 64) {
      const batch = await Promise.all(dirents.slice(start, start + 64).map(async entry => {
        ...
        try {
          ...
          return { id: grant(filePath, type, true), name: entry.name, type, size: stat.size, modifiedAt: stat.mtime.toISOString() }
        } catch {
          return null
        }
      }))
  ...
  /** 只接受当前列表中的文件/文件夹令牌；父目录和当前位置的导航令牌不能用于上传。 */
  selectedPaths (ownerId, fileIds) {
    ...
    return fileIds.map(id => {
      ...
      if (!entry?.uploadable || !['file', 'directory'].includes(entry.type)) throw new Error('本地文件引用已失效，请刷新后重新选择')
      return entry.path
    })
  }
