...
async function writeRemoteFile (sftp, destinationPath, createSource) {
  try {
...
class SftpManager {
  ...
  /** Opens one strictly host-verified SFTP channel with an ephemeral credential. */
  async connect (ownerId, storedProfile, secret = '') {
    ...
    try {
      ...
      if (!this.connections.has(connectionId)) throw new Error('SFTP connection was closed')
  ...
  async upload (ownerId, connectionId, remoteDirectory, localPath) {
    const record = this.requireOwned(ownerId, connectionId)
    ...
    // 对话框和真实 File 拖放共用同一传输路径及同名保护。
    await writeRemoteFile(record.sftp, remotePath, () => createReadStream(sourcePath))
